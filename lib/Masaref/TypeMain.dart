...
//
                ...
                children: [
                  ...
                  Row(
                    children: [
                       ...
                       _selectedCat = await showMonthYearPicker(
  context: context,
  ...
  lastDate: DateTime(2023),
                      ...
                      //             items: list_cat_year.map((String val) {
                      //               return new DropdownMenuItem<String>(
                      //                 value: val,
                      //                 child: new Text(val),
                      //               );
                      //             }).toList(),
                      //             hint: Text(_selectedCatyear),
                      //             onChanged: (newVal) {
                      //               this.setState(() {
                      //                 _selectedCatyear = newVal;
                      //                 print(
                      //                     '_selectedCatyear$_selectedCatyear');
                      //                 filterSearchMonth();
                      //               });
                      //             }),
                      //       ),
  ...
  printlist() {
    if (cars != null) {
      ...
      for (var i = 0; i < cars.docs.length; i++) {
        ...
        if (cars.docs[i].data()['type_entry_date'] != null) {
          temp_date = cars.docs[i]
              .data()['type_entry_date']
              .toString()
              .substring(0, 10);
        } else {
    ...
    Typeslist.sort(
        (a, b) => int.parse(a['type_id']).compareTo(int.parse(b['type_id'])));
    var array_len = Typeslist.length;
    setState(() {
      prod_max =
          ((int.parse(Typeslist[array_len - 1]['type_id']) + 1).toString());
      Typeslist.sort(
          (b, a) => int.parse(a['type_id']).compareTo(int.parse(b['type_id'])));
              ...
              Navigator.of(context).push(
                new MaterialPageRoute(
                    builder: (BuildContext context) => new TypesDetails(
                          ...
                          type_doc_id:
                              Typeslist[index]['type_doc_id'].toString(),
                        )),
              );
...
*/
  ...
  void filterSearchMonth() {
    print(duplicateItems);
    dummySearchList = duplicateItems;
    if (_selectedCatyear.isNotEmpty && _selectedCatyear != 'All') {
      // var now = new DateTime.now();
      var st_month = new DateTime(
          int.parse(_selectedCatyear), int.parse(_selectedCat), 01);
      var next_month = new DateTime(
          int.parse(_selectedCatyear), int.parse(_selectedCat) + 1, 01);
      int a = int.parse(_selectedCat);
      if (a >= 10) {
        _selectedCat = _selectedCat.toString();
      } else {
        _selectedCat = '${'0'}' + '${_selectedCat}';
      }
      dummyListData.clear();
      dummySearchList.forEach((item) {
